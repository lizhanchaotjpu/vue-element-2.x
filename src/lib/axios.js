...
import { dataCode, tokenName, timeOut, apiServer } from '@/conf/config'
...
// 设置认证信息
axios.defaults.headers[tokenName] = stores.get(tokenName) || ''
// 数据接口域名统一配置
...
axios.interceptors.response.use(
  (response) => {
    if (response.data) {
      if (response.data.code === dataCode.invalidCode) {
        stores.remove(tokenName)
        router.push('/login')
      }
    }
    return response
